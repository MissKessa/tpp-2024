...
namespace TPP.Laboratory.Functional.Lab08
{
    ...
    class Query
    {
		...
		private void Query5()
        {
            ...
            Console.WriteLine();
            Console.WriteLine("*** *** *** QUERY 5: Counting *** *** ***");
            Console.WriteLine();
            var q6 = model.Employees.GroupBy(e => e.Province).ToDictionary(x => x.Key, x => x.Count());
            foreach (var prov in q6)
            {
                Console.WriteLine(prov.Key+" "+prov.Value);
            }
        }
        ...
        private void Homework2()
        {
            ...
            var h3 = h2.Aggregate(0, (acc, x) => { acc += x.p.Seconds; return acc; });
