...
namespace TPP.Concurrency.Threads {
    ...
    public class Master {
        ...
        public Master(int[] A, int[] B, int numberOfThreads) {
            if (numberOfThreads < 1 || numberOfThreads > A.Length)
                throw new ArgumentException("The number of threads must be lower or equal to the elements of the vector");
            this.A = A;
            this.B = B;
            this.numberOfThreads = numberOfThreads;
        }
        public List<int> ComputeDifference()
        {
            difference = new List<int>();
            // * Workers are created
            Worker[] workers = new Worker[this.numberOfThreads];
            int elementsPerThread = this.A.Length / numberOfThreads;
            for (int i = 0; i < this.numberOfThreads; i++)
                workers[i] = new Worker(this.A,this.B, difference,
                    i * elementsPerThread,
                    (i < this.numberOfThreads - 1) ? (i + 1) * elementsPerThread - 1 : this.A.Length - 1 // last one
                    );
