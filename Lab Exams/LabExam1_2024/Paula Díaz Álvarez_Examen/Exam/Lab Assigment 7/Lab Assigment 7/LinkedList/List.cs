...
namespace LinkedList
{
    ...
    public partial class List<T> : IEnumerable<T>
    {
        ...
        public T Find(Predicate<T> p) //First
        {
            foreach (T t in this)
            {
                if (p(t)) return t;
        ...
        public List<T> Filter(Predicate<T> p) //Where
        {
        ...
        public W Reduce<W>(Func<W, T, W> func, W seed = default(W)) //Aggregate
        {
            W result = func(seed, GetElement(0));
            for (int i = 1; i < NumberOfElements; i++)
            {
                result = func(result, GetElement(i));
            }
            return result;
        }

        public List<T> Invert() //Reverse
        {
            List<T> result = new List<T>();
        ...
        public List<W> Map<W>(Func<T,W> func) //Select
        {
            List<W> result = new List<W>();
            for (int i = 0; i<NumberOfElements; i++)
            {
                result.Add(func(GetElement(i)));
            }
            return result;
        }
